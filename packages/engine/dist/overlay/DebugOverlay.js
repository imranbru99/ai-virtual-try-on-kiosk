...
class DebugOverlay {
    ...
    render(context, detection, metrics) {
        ...
        if (this.options.showBoundingBoxes && detection.poseLandmarks) {
            const box = (0, geometry_js_1.computeLandmarksBoundingBox)(detection.poseLandmarks, width, height);
    ...
    drawLandmarkDots(ctx, landmarks, w, h, color) {
        ctx.fillStyle = color;
